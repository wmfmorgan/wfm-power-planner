...
from app.services.data_service import export_all, import_all

@goals_bp.route('/api/export')
@login_required
def export_data():
    data = data_service.export_all(current_user.id, current_user.username)
    timestamp = datetime.utcnow().strftime('%Y-%m-%d_%H%M')
    filename = f'wfm-power-planner-full-backup_{timestamp}.json'
    return (
        jsonify(data),
        200,
        {
...
def import_data():
    ...
    try:
        data = json.load(file)
        import_all(data, current_user.id)
        return jsonify({"message": "Full empire restored. Hulkamania runs wild again."}), 200
    except Exception as e:
        current_app.logger.error(f"Full import failed: {e}")
